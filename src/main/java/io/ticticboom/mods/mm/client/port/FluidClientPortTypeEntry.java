...
public class FluidClientPortTypeEntry extends ClientPortTypeEntry {
    ...
    public void renderScreen(PortStorage storage, PortScreen screen, PoseStack ms, int x, int y) {
        ...
        if (!storg.handler.stack().isEmpty()) {
            ...
            GuiComponent.drawCenteredString(ms, Minecraft.getInstance().font, storg.handler.stack().getAmount() + " " + storg.handler.stack().getDisplayName().getString(), screen.getGuiLeft() + startX + 9, screen.getGuiTop() + startY + 30, 0xfefefe);
        }
    }
}
